...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        for (int i = 0; i < humanRelationships.length; i++) {
            if (humanRelationships[Math.max(i, index)][Math.min(i, index)]) {
                ...
                friendsAndPotentialFriends.addAll(getAllFriendsAndPotentialFriends(i, deep - 1));
            }
        }
    ...
    public Set<Integer> removeFriendsFromSet(Set<Integer> set, int index) {
        for (int i = 0; i < humanRelationships.length; i++) {
            if (index < humanRelationships.length && humanRelationships[Math.max(i, index)][Math.min(i, index)]) {
                set.remove(i);
            }
        }
        return set;
